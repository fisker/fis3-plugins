...
module.exports = function (content, file, conf) {
  ...
  var promise = (0, _postcss.default)([_stylelint.default]).process(content, config).then(function (result) {
    ...
    for (var i = 0; i < messages.length; i++) {
      ...
      (type === 'error' ? errorMsg : warnMsg).push([' ', "".concat(type, ":"), message.line ? "[".concat(message.line, ":").concat(message.column, "]") : '', // '[' + message.rule + ']',
      message.text].join(' '));
    }

    if (warnMsg.length || errorMsg.length) {
      log.warn('[%s] lint failed: \n%s \n  %s problem (%s errors, %s warning)', file.id, warnMsg.concat(errorMsg).join('\n'), warnMsg.length + errorMsg.length, errorMsg.length, warnMsg.length);
