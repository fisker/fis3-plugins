...
var _browserSyncConfig = _interopRequireDefault(require("./lib/browser-sync-config.js"));
...
function now() {
  var d = new Date();
  var str = [d.getHours(), d.getMinutes(), d.getSeconds()].join(':').replace(/\b\d\b/g, '0$&');
...
function onInit(config) {
  return function () {
    console.log('Listening on %s://127.0.0.1:%d', config.https ? 'https' : 'http', config.port);
  };
}
...
function replaceScriptTag(bs) {
  ...
  bs.instance.config.templates.scriptTag = _path.default.join(__dirname, 'templates/script-tags.tmpl');
}
